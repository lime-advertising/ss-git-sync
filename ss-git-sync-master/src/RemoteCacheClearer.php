...
class RemoteCacheClearer {
    ...
    /**
     * @param array $targets Labels of secondary sites selected for cache clear.
    ...
    public function trigger(array $targets): array {
        ...
        foreach ($targets as $label) {
            ...
            if (($result['status'] ?? '') === 'success') {
                ...
                $failed[$label] = $result['message'] ?? __('Cache clear failed.', 'ssgs');
            }
        }
    ...
    protected function dispatch(array $secondary, string $secret): array {
        ...
        if ($code >= 200 && $code < 300) {
            ...
            Logger::log(
                'distributor',
                sprintf(
                    'Remote cache clear request sent to %s (%s). Status: %s',
                    $secondary['label'],
                    $endpoint,
                    $status
                )
            ...
            if ($status === 'success') {
                return [
                    'status'  => 'success',
                    'message' => $message !== '' ? $message : __('Cache cleared.', 'ssgs'),
                ];
            }
    ...
    private function buildEndpoint(string $baseUrl): ?string {
        ...
        return trailingslashit($baseUrl) . 'wp-json/ssgs/v1/clear-cache';
    }
}
